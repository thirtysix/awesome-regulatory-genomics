...
def main() -> None:
    ...
    ap.add_argument("--recheck-flagged", action="store_true",
                    help="re-derive the paper for records the describe stage flagged "
                         "in llm_proposals.yaml: paper_mismatch, and compare it with "
                         "the one on record")
    args = ap.parse_args()
    ...
    if args.recheck_flagged:
        # Records that HAVE a publication which the describe stage judged
        # unrelated to the tool. The method is unchanged and deliberately so:
        # ask the tool what to cite, never search the literature for its name.
        # A wrong paper and a missing paper are found the same way, and this is
        # how eight records were corrected before - MoonlightR was carrying a
        # spine-surgery review, vulcan was carrying VIPER's 1,079 citations.
        flagged = set((yaml.safe_load((CURATION / "llm_proposals.yaml").read_text()) or {})
                      .get("paper_mismatch") or {})
        targets = [t for t in tools if t["id"] in flagged and t["id"] not in settled]
        print(f"{len(targets)} records were flagged as carrying an unrelated paper")
    else:
    ...
    if args.recheck_flagged:
        # Compare what the tool declares with what the catalog records. Compare
        # WORKS, not identifier strings: a PMID and a DOI for one paper are not a
        # disagreement, and 14 of 50 apparent conflicts were exactly that.
        by_id = {t["id"]: t for t in tools}
        agree = differ = nothing = 0
        lines = ["# Re-derived papers for records flagged as carrying the wrong one", "",
        ...
        for r in sorted(rows, key=lambda r: r["name"].lower()):
            t = by_id.get(r["id"], {})
            rec = t.get("publication") or ""
            got = (f"doi:{r['doi']}" if r.get("doi")
                   else f"pmid:{r['pmid']}" if r.get("pmid") else "")
            if not got:
                nothing += 1
                same = "-"
            else:
                # same work if either identifier string matches, or both resolve
                # to the same recorded pmid/doi pair on the catalog row
                same_doi = r.get("doi") and t.get("doi") and \
                    r["doi"].lower().rstrip("/") == str(t["doi"]).lower().rstrip("/")
                same_pmid = r.get("pmid") and t.get("pmid") and \
                    str(r["pmid"]) == str(t["pmid"])
                if got == rec or same_doi or same_pmid:
                    agree += 1; same = "yes"
                else:
                    differ += 1; same = "**NO**"
            lines.append(f"| `{r['id']}` {r['name']} | {rec or '-'} | {got or '-'} | "
                         f"{same} | {r['verdict']}: {r['why']} |")
        lines[10:10] = [
            f"{len(rows)} flagged records re-checked: **{differ} disagree** with what the",
            f"tool itself declares, {agree} agree (so the flag was a false positive), and",
            f"{nothing} declare nothing anywhere, leaving the flag unresolved.", ""]
        report = DOCS / "publication-recheck.md"
        report.write_text("\n".join(lines) + "\n")
        print(f"  {differ} disagree, {agree} agree, {nothing} declare nothing")
        print(f"  -> {report}")
        return
