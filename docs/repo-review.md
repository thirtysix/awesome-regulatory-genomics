...
Generated 2026-08-02 by `make repos`.
...
- **245 held for review** below; none of these are applied.
- **19 records yielded no candidate at all** from any source. These are cached as such so the monthly refresh does not re-fetch the same third-party homepages to learn nothing new; `--refresh` reconsiders them.
...
| ARGO | [xinehc/argo](https://github.com/xinehc/argo) | bioconda | name matches but only 0 shared terms () | Argo: species-resolved profiling of antibiotic resistance genes in complex meta… |
...
| AUREA | [AEPAX/VerbaAurea](https://github.com/AEPAX/VerbaAurea) | github-search | no name match and only 0 shared terms () | VerbaAurea： 一个文档预处理工具，专注于为知识库构建提供高质量的文本数据。 |
...
| BindSpace | [hy395/BindSpace](https://github.com/hy395/BindSpace) | github-search | name matches but only 1 shared terms (embedding) | TFBS models trained with HT-SELEX data using StarSpace embedding method |
| BioSwitch | [google/safevalues](https://github.com/google/safevalues) | homepage | no name match and only 2 shared terms (google, https) |  |
...
| BiSA | [Bisa/factorio-init](https://github.com/Bisa/factorio-init) | github-search | no name match and only 0 shared terms () | Factorio init script |
...
| CARMEN | [mdredze/carmen-python](https://github.com/mdredze/carmen-python) | pypi | no name match and only 0 shared terms () | Geolocation for Twitter. |
...
| ChAMP | [wweir827/champ](https://github.com/wweir827/champ) | pypi | name matches but only 0 shared terms () |  |
...
| ChIPComp | [EinerderIdioten/ChipCompAgent](https://github.com/EinerderIdioten/ChipCompAgent) | github-search | no name match and only 0 shared terms () | This is an AI agent to compare the baselines of different type of AI chips. It… |
...
| chroGPS | [bioconductor-source/chroGPS](https://github.com/bioconductor-source/chroGPS) | github-search | name matches but only 0 shared terms () |  |
...
| Cogito | [freepik-company/fc-py-cogito](https://github.com/freepik-company/fc-py-cogito) | pypi | no name match and only 1 shared terms (cogito) |  |
| CoMeBack | [agamm/comeback](https://github.com/agamm/comeback) | pypi | name matches but only 0 shared terms () | Project restoration in one command, auto open everything! |
| ComHub | [whatwg/fetch](https://github.com/whatwg/fetch) | homepage | no name match and only 0 shared terms () | Fetch Standard |
| COUGER | [couger-inc/cream](https://github.com/couger-inc/cream) | github-search | no name match and only 0 shared terms () | zkCREAM is zk-SNARK based anonymized voting application using a token mixer |
| createcontrolsubset | [samtools/htslib](https://github.com/samtools/htslib) | homepage | no name match and only 0 shared terms () | C library for high-throughput sequencing data formats |
...
| csRNA-seq | [annamcd511/csRNA-seq_WalkThru](https://github.com/annamcd511/csRNA-seq_WalkThru) | github-search | no name match and only 0 shared terms () |  |
| CSSQ | [Tatsh/cssq](https://github.com/Tatsh/cssq) | pypi | name matches but only 0 shared terms () | Filter HTML with a CSS query |
| CUDA-MEME | [angelfloreso/CUDA-Memetic-Algorithm](https://github.com/angelfloreso/CUDA-Memetic-Algorithm) | github-search | no name match and only 2 shared terms (algorithm, cuda) | CUDA code for DE+PSO for Gene Regulatory Network inference |
...
| CWAS-Plus | [sanderslab/cwas](https://github.com/sanderslab/cwas) | homepage | no name match and only 1 shared terms (category) |  |
...
| DANPOS | [google/safevalues](https://github.com/google/safevalues) | homepage | no name match and only 0 shared terms () |  |
...
| DeFCoM | [MarisaKirisame/DefComBlog](https://github.com/MarisaKirisame/DefComBlog) | github-search | no name match and only 0 shared terms () |  |
...
| figeno | [CompEpigen/figeno](https://github.com/CompEpigen/figeno) | pypi | name matches but only 1 shared terms (data) | Tool for plotting sequencing data along genomic coordinates. |
| FIRE | [google/python-fire](https://github.com/google/python-fire) | pypi | no name match and only 0 shared terms () | Python Fire is a library for automatically generating command line interfaces (… |
...
| FunciSNP | [michellerjones/FunciSNP](https://github.com/michellerjones/FunciSNP) | github-search | repo has no description; cannot verify beyond the name |  |
| FunSiP | [intuitivelabs/funsip](https://github.com/intuitivelabs/funsip) | github-search | name matches but only 0 shared terms () | SIP proxy in go with JS routing logic |
...
| Geneious | [SmithsonianWorkshops/GeneiousLIMS](https://github.com/SmithsonianWorkshops/GeneiousLIMS) | github-search | no name match and only 0 shared terms () | Materials for a series on how to use Geneious and its LIMS plugin to document t… |
...
| GGEA | [clement-alexandre/TotemBionet](https://github.com/clement-alexandre/TotemBionet) | pypi | no name match and only 0 shared terms () | Interactive Jupyter notebook for an accessible and reproducible computational a… |
...
| GINsim | [GINsim/GINsim-python](https://github.com/GINsim/GINsim-python) | pypi | no name match and only 0 shared terms () | Python bindings for GINsim and bioLQM |
| GPMiner | [LHanLi/GPminer](https://github.com/LHanLi/GPminer) | pypi | repo has no description; cannot verify beyond the name |  |
...
| GRENITS | [ElEd2/GRENITS](https://github.com/ElEd2/GRENITS) | github-search | repo has no description; cannot verify beyond the name |  |
| GROK | [zopefoundation/grok](https://github.com/zopefoundation/grok) | pypi | name matches but only 0 shared terms () | Grok: Now even cavemen can use Zope 3! |
...
| homer | [npdeloss/mepp](https://github.com/npdeloss/mepp) | homepage | no name match and only 2 shared terms (dna, motif) | Motif Enrichment Positional Profiling (MEPP) quantifies a positional profile of… |
| ICEberg | [iceberg-project/iceberg-middleware](https://github.com/iceberg-project/iceberg-middleware) | pypi | no name match and only 0 shared terms () | This repo contains the ICEBERG middleware as it is agreed by the members of the… |
...
| ISMA | [Mufanc/iSmartAuto](https://github.com/Mufanc/iSmartAuto) | github-search | no name match and only 0 shared terms () | ✨全新思路✨ \| iSmart 刷课工具，自动完成任务，一分钟一门课 |
| JASPAR2018 | [bioc/JASPAR2018](https://github.com/bioc/JASPAR2018) | github-search | name matches but only 0 shared terms () | This is a read-only mirror of the git repos at https://bioconductor.org |
...
| lpNet | [xiaoneil/LPNet](https://github.com/xiaoneil/LPNet) | github-search | name matches but only 0 shared terms () |  |
...
| MARGE | [suwangbio/MARGE_py3](https://github.com/suwangbio/MARGE_py3) | bioconda | name matches but only 0 shared terms () | MARGE:Model-based Analysis of Regulation of Gene Expression (for python3) |
...
| matrix-clustering | [TheLoneNut/CorrelationMatrixClustering](https://github.com/TheLoneNut/CorrelationMatrixClustering) | github-search | no name match and only 0 shared terms () | An example on how Correlation Matrix can be displayed and clustered |
| MCOT | [yaotingwangofficial/Awesome-MCoT](https://github.com/yaotingwangofficial/Awesome-MCoT) | github-search | no name match and only 1 shared terms (comprehensive) | Multimodal Chain-of-Thought Reasoning: A Comprehensive Survey |
...
| MEDUSA | [combogenomics/medusa](https://github.com/combogenomics/medusa) | bioconda | name matches but only 0 shared terms () | A draft genome scaffolder that uses multiple reference genomes in a graph-based… |
...
| MEME Suite | [icaoberg/docker-meme-suite](https://github.com/icaoberg/docker-meme-suite) | github-search | no name match and only 1 shared terms (discovery) | :whale: :computer: Docker recipe for meme-suite |
...
| miRvestigator | [cplaisier/miRvestigator](https://github.com/cplaisier/miRvestigator) | homepage | name matches but only 1 shared terms (identify) | Identify miRNA that binds to a PSSM motif from 3' UTR. |
| MixChIP | [malonzm1/LuxHMM](https://github.com/malonzm1/LuxHMM) | homepage | no name match and only 2 shared terms (probabilistic, seque… |  |
...
| MoAn | [NS-Sp4ce/MoAn_Honey_Pot_Urls](https://github.com/NS-Sp4ce/MoAn_Honey_Pot_Urls) | github-search | no name match and only 0 shared terms () | X安蜜罐用的一些存在JSonp劫持的API |
| MochiView | [tdseher/mochiview2gff](https://github.com/tdseher/mochiview2gff) | github-search | no name match and only 0 shared terms () | Python script to convert MochiView annotation file to GFF3 format |
...
| PANDA | [davidvi/pypanda](https://github.com/davidvi/pypanda) | homepage | name matches but only 1 shared terms (passing) | Python implementation of PANDA (Passing Attributes between Networks for Data As… |
...
| Possum | [brysontyrrell/Possum](https://github.com/brysontyrrell/Possum) | pypi | name matches but only 0 shared terms () | A packaging tool for Python based AWS serverless applications. |
| POXO | [sashakile/poco](https://github.com/sashakile/poco) | pypi | no name match and only 0 shared terms () | Small and lazy panda |
...
| PREDICT | [Svdvoort/PREDICTFastr](https://github.com/Svdvoort/PREDICTFastr) | pypi | no name match and only 1 shared terms (predict) |  |
...
| PRP | [brettatoms/prp](https://github.com/brettatoms/prp) | pypi | name matches but only 1 shared terms (prp) |  |
...
| REACTR | [natlee-the-anteel/reactrv1.0](https://github.com/natlee-the-anteel/reactrv1.0) | homepage | name matches but only 0 shared terms () | reactrv1.0 is a modular bioinformatics pipeline that automates exploration and… |
...
| regCNN | [shinjan025/Pressure_predictor_ReGCNN_Drivernet](https://github.com/shinjan025/Pressure_predictor_ReGCNN_Drivernet) | github-search | no name match and only 0 shared terms () | Modify the ReGCNN from the original authors of Drivernet, to predict pressure d… |
...
| RNA-targeting | [ArcInstitute/RNAtargeting_web_custom](https://github.com/ArcInstitute/RNAtargeting_web_custom) | homepage | no name match and only 2 shared terms (custom, guide) | source code of the custom page for the RNA targeting website - Cas13d guide des… |
...
| scATAC-seq | [ENCODE-DCC/atac-seq-pipeline](https://github.com/ENCODE-DCC/atac-seq-pipeline) | homepage | no name match and only 2 shared terms (atac, seq) | ENCODE ATAC-seq pipeline |
...
| SELEX | [ren-lab/snp-selex](https://github.com/ren-lab/snp-selex) | github-search | name matches but only 2 shared terms (data, selex) |  |
...
| SEME | [missing-semester-cn/missing-semester-cn.github.io](https://github.com/missing-semester-cn/missing-semester-cn.github.io) | github-search | no name match and only 0 shared terms () | the CS missing semester Chinese version |
...
| Snapper | [DNKonanov/snapper-man-test](https://github.com/DNKonanov/snapper-man-test) | homepage | no name match and only 0 shared terms () | Readthedocs manual for Snapper |
...
| STREME | [streme-fun/streme-frontend](https://github.com/streme-fun/streme-frontend) | github-search | no name match and only 1 shared terms (streme) |  |
...
| T-Gene | [huggingface/text-generation-inference](https://github.com/huggingface/text-generation-inference) | github-search | no name match and only 0 shared terms () | Large Language Model Text Generation Inference |
...
| TFPred | [vivekka93/tfpred](https://github.com/vivekka93/tfpred) | pypi | name matches but only 0 shared terms () | Use tensorflow model to run predictions with resource constraints |
...
| TSSFinder | [tssfinder/tssfinder.github.io](https://github.com/tssfinder/tssfinder.github.io) | homepage | no name match and only 2 shared terms (prediction, tssfinde… |  |
...
| VARAdb | [varadbhogayata/varadbhogayata.github.io](https://github.com/varadbhogayata/varadbhogayata.github.io) | github-search | no name match and only 0 shared terms () | My Portfolio - Personal Website |
...
| VISDB | [chhankyao/visdb](https://github.com/chhankyao/visdb) | github-search | name matches but only 2 shared terms (human, visdb) |  |
| VISION | [guanjue/IDEAS_2018](https://github.com/guanjue/IDEAS_2018) | homepage | no name match and only 0 shared terms () | Jointly characterizing epigenetic dynamics across multiple cell types |
| WashU Epigenome Browser | [twlab/eg3](https://github.com/twlab/eg3) | homepage | no name match and only 3 shared terms (browser, epigenome,… | WashU Epigenome Browser (3rd generation, since 2025) |
